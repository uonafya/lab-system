<script type="text/javascript">
    $(document).ready(function() {
        ...
        set_select_patient("sidebar_covidpatient_search", "{{ url('/covid_patient/search') }}", 2,
            "Search for Covid-19 patient", true);
        set_select_patient("sidebar_covidpatient_nat_id_search", "{{ url('/covid_patient/nat_id') }}", 2,
            "Search for Covid-19 Nat-ID", true);

        @if (in_array(env('APP_LAB'), [1]))
            set_select_patient("sidebar_covid_kemri_id_search", "{{ url('/covid_sample/kem_id') }}", 2,
                "Search for KEMRI ID", true);
        @elseif
            (in_array(env('APP_LAB'), [25]))
            set_select_patient("sidebar_covid_kemri_id_search", "{{ url('/covid_sample/kem_id') }}", 2,
                "Search for AMREF ID", true);
        @endif
        set_select("sidebar_covidlabID_search", "{{ url('/covid_sample/search') }}", 1,
            "Search by Covid-19 Lab ID");
        set_select("sidebar_covid_worksheet_search", "{{ url('/covid_worksheet/search') }}", 1,
            "Search for Covid-19 worksheet", true);
        ...
        set_select_patient("viralpatient_search", "{{ url('/viralpatient/search') }}", 2,
            "Search for patient");

        set_select_patient("sidebar_patient_search", "{{ url('/patient/search') }}", 2,
            "Search for EID patient", true);
        set_select_patient("sidebar_viralpatient_search", "{{ url('/viralpatient/search') }}", 2,
            "Search for VL patient", true);
        set_select("sidebar_cd4_patientname", "{{ url('/cd4/patient/search_name') }}", 2,
            "Search for patient name", false, true);
        set_select("sibebar_cd4medrecNo_search", "{{ url('cd4/patient/search_record_no') }}", 2,
            "Search Medical Record #(Ampath #)", false, true);
        ...
        set_select("viralworksheet_search", "{{ url('/viralworksheet/search') }}", 1, "Search for worksheet",
            true);
        set_select("sidebar_cd4worksheet_search", "{{ url('cd4/worksheet/search') }}", 1,
            "Search for Worksheet");

        set_select("sidebar_worksheet_search", "{{ url('/worksheet/search') }}", 1,
            "Search for EID worksheet", true);
        set_select("sidebar_viralworksheet_search", "{{ url('/viralworksheet/search') }}", 1,
            "Search for VL worksheet", true);

        set_select_facility("facility_search", "{{ url('/facility/search') }}", 3, "Search for facility",
            "{{ url('/batch/facility') }}");
        set_select_facility("sidebar_facility_search", "{{ url('/facility/search') }}", 3,
            "Search for facility batches", "{{ url('/batch/facility') }}");
        set_select_facility("sidebar_viralfacility_search", "{{ url('/facility/search') }}", 3,
            "Search for facility batches", "{{ url('/viralbatch/facility') }}");
        set_select_facility("sidebar_dr_facility_search", "{{ url('/facility/search') }}", 3,
            "Search for facility samples", "{{ url('/dr_sample/facility') }}");
        set_select_facility("sidebar_cd4facility_search", "{{ url('/facility/search') }}", 3,
            "Search Site Samples", "{{ url('/cd4/sample/facility') }}")

        set_select("sidebar_labID_search", "{{ url('sample/search') }}", 1, "Search by EID Lab ID");
        set_select("sidebar_recencyNo_search", "{{ url('sample/search_recency_no') }}", 1,
            "Search by recency No");
        set_select("sidebar_virallabID_search", "{{ url('viralsample/search') }}", 1, "Search by VL Lab ID");


        set_select_orderno("sidebar_order_no_search", "{{ url('sample/ord_no') }}", 1,
            "Search by EID Order No");
        set_select_orderno("sidebar_viral_order_no_search", "{{ url('viralsample/ord_no') }}", 1,
            "Search by VL Order No");
        set_select("sidebar_cd4labID_search", "{{ url('cd4/sample/search') }}", 1, "Search by CD4 Lab ID");


        set_select_patient("dr_patient_search", "{{ url('/viralpatient/search') }}", 2,
            "Search for DR patient", "{{ url('/viralpatient/dr') }}");
        set_select_patient("dr_nat_id_search", "{{ url('/viralpatient/nat_id') }}", 2,
            "Search for DR nat ID", "{{ url('/viralpatient/dr') }}");
