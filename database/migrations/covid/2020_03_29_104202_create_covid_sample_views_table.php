...
class CreateCovidSampleViewsTable extends Migration
{
    ...
    {        
        ...
        (
          SELECT s.*, p.facility_id, p.quarantine_site_id, p.case_id, p.county_id, p.identifier_type, p.identifier, p.patient_name, p.occupation, p.phone_no, p.justification, p.county, p.subcounty, p.ward, p.residence, p.hospital_admitted, p.dob, p.sex, p.current_health_status, p.date_symptoms,  p.date_admission, p.date_isolation, date_death, `f`.`facilitycode`,`f`.`name` as facilityname, f.partner, f.district, qs.name as quarantine_site, c.name as countyname, d.name as subcountyname
