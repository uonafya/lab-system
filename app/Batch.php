...
class Batch extends BaseModel
{
    ...
    public function scopeEligible($query, $facility, $datereceived)
    {
        ...
        if(!$datereceived){
            return $query->where(['facility_id' => $facility, 'user_id' => $user_id, 'batch_full' => 0, 'batch_complete' => 0, 'input_complete' => 0])
                    ->whereDate('created_at', $today)->whereNull('datereceived')->whereNull('datedispatched');
        }
        return $query->where(['facility_id' => $facility, 'datereceived' => $datereceived, 'user_id' => $user_id, 'batch_full' => 0, 'batch_complete' => 0, 'input_complete' => 0])->where('created_at', '>', $min_date)->whereNull('datedispatched');
    }
