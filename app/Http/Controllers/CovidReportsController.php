	...
	private function prepareData($today_data, $last_update_data, $date)
	{
		...
		if(env('APP_LAB') == 1 && auth()->user()->lab_id == env('APP_LAB')){
			...
			foreach ($labs as $key => $lab) {
				$today_data_other = $this->get_model($lab->lab_id)->whereDate('datetested', $date)->orderBy('result', 'desc')->get();
				$last_update_data_other = $this->get_model($lab->lab_id)->whereDate("datetested", '<', $date)->get();

				$data[] = $this->get_summary_data($today_data_other, $last_update_data_other, $date, $lab->lab_id);
				$data[] = [$key, $lab->lab_id, $lab->toJson(), Lab::find($lab->lab_id)->toJson()];
			}
		}
