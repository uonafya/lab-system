...
class Misc extends Common
{
    ...
    public static function check_patients_list(){
        ...
        $found = [];
        $missmatch = [];
        // $patients = \App\SampleView::selectRaw("distinct patient, datetested")->whereNotIn('patient', $patientsList)->whereBetween('datetested', ['2018-05-01','2018-11-30'])->where('result', '=', 2)->get();

        $patients = \App\SampleView::selectRaw("count(distinct patient)")->whereBetween('datetested', ['2018-05-01','2018-11-30'])->where('result', '=', 2)->get();
        dd($patients);
        foreach ($patients as $key => $patient) {
            $found[] = $patient->patient. ' ----- '. $patient->datetested;
        }
